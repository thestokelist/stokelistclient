...
function PostCreate() {
    ...
    const onSubmit = async data => {
        ...
        let price = (data.price === "") ? (data.priceRadio === "priceFree" ? "Free" : "") : data.price;
        let location = data.location
        let exactLocation = (data.lng !== "" && data.lat !== "") ? {
            "type": "Point",
            "coordinates": [data.lng, data.lat]
        } : null
        let email = data.email
        const postData = {title, description, price, location, exactLocation, email}
    ...
    return (<div>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    />
                    {errors.title && errors.title.type === "required" && 'Title is required.'}
                    {errors.title && errors.title.type === "maxLength" && 'Title has a max length of 50 characters.'}
                    ...
                    $<Input type="numeric" name="price" ref={register({validate: validatePrice})} onChange={updatePriceForm}/>
                    <Input type="radio" name="priceRadio" value="priceNA" ref={register()} onChange={updatePriceForm}/>Not Applicable 
                    <Input type="radio" name="priceRadio" value="priceFree" ref={register()} onChange={updatePriceForm}/>Free
                    <br />{errors.price && 'Enter a price or select an option'}
                    ...
                    <Input type="email" name="email" ref={register({ required: true, validate: validateEmail })} />
                    {errors.email && errors.email.type === "required" && 'Email is required.'}
                    {errors.email && errors.email.type === "validate" && 'Must be an email address.'}
